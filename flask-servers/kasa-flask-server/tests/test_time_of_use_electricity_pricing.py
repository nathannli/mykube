...
class TestWinterDateRange:
    ...
    def test_is_winter_during_november(self, mock_datetime):
        """Test is_winter returns True during November."""
        # November 15 - clearly in winter season
        current_year = datetime.now().year
    ...
    def test_is_winter_during_december(self, mock_datetime):
        """Test is_winter returns True during December."""
        # December 15 - clearly in winter season
        current_year = datetime.now().year
    ...
    def test_is_winter_at_boundary_start(self, mock_datetime):
        """Test is_winter returns True on November 1."""
    ...
    def test_is_winter_on_april_15(self, mock_datetime):
        """Test is_winter returns True on April 15 (within winter season)."""
    ...
    def test_is_not_winter_in_summer(self, mock_datetime):
        """Test is_winter returns False during summer."""
        # June 15
    ...
    def test_is_not_winter_after_april_30(self, mock_datetime):
        """Test is_winter returns False on May 1."""
        # May 1
        mock_now = datetime(2025, 5, 1)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.is_winter() is False
    ...
    def test_is_not_winter_before_november_1(self, mock_datetime):
        """Test is_winter returns False on October 31."""
        # October 31
        mock_now = datetime(2024, 10, 31)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.is_winter() is False
...
class TestWeekdayDetection:
    ...
    def test_is_weekday_on_monday(self, mock_datetime):
        """Test is_weekday returns True on Monday."""
    ...
    def test_is_weekday_on_wednesday(self, mock_datetime):
        """Test is_weekday returns True on Wednesday."""
    ...
    def test_is_weekday_on_friday(self, mock_datetime):
        """Test is_weekday returns True on Friday."""
    ...
    def test_is_not_weekday_on_saturday(self, mock_datetime):
        """Test is_weekday returns False on Saturday."""
    ...
    def test_is_not_weekday_on_sunday(self, mock_datetime):
        """Test is_weekday returns False on Sunday."""
...
class TestGetCurrentPrice:
    """Test get_current_price method with various scenarios."""

    @patch('time_of_use_electricity_pricing.datetime')
    def test_winter_weekday_off_peak_morning(self, mock_datetime):
        """Test winter weekday off-peak morning rate."""
        # Monday, November 8, 2025, 6:00 AM (winter, weekday, hour 6)
        current_year = datetime.now().year
        mock_now = datetime(current_year, 11, 10, 6, 0, 0)  # Monday in November
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
    ...
    def test_winter_weekday_peak_morning(self, mock_datetime):
        """Test winter weekday peak morning rate (7-10)."""
        # Monday, November 10, 2025, 9:00 AM (winter, weekday, hour 9)
        current_year = datetime.now().year
        mock_now = datetime(current_year, 11, 10, 9, 0, 0)  # Monday in November
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
    ...
    def test_winter_weekday_partial_peak_afternoon(self, mock_datetime):
        """Test winter weekday partial peak afternoon rate (11-16)."""
        # Monday, November 10, 2025, 2:00 PM (winter, weekday, hour 14)
        current_year = datetime.now().year
        mock_now = datetime(current_year, 11, 10, 14, 0, 0)  # Monday in November
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
    ...
    def test_winter_weekday_peak_evening(self, mock_datetime):
        """Test winter weekday peak evening rate (17-18)."""
        # Monday, November 10, 2025, 5:00 PM (winter, weekday, hour 17)
        current_year = datetime.now().year
        mock_now = datetime(current_year, 11, 10, 17, 0, 0)  # Monday in November
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.203
    ...
    def test_winter_weekday_off_peak_night(self, mock_datetime):
        """Test winter weekday off-peak night rate."""
        # Monday, November 10, 2025, 10:00 PM (winter, weekday, hour 22)
        current_year = datetime.now().year
        mock_now = datetime(current_year, 11, 10, 22, 0, 0)  # Monday in November
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.098
    ...
    def test_winter_weekend_flat_rate(self, mock_datetime):
        """Test winter weekend has flat rate all day."""
        # Saturday, November 8, 2025, 2:00 PM (winter, weekend, hour 14)
        current_year = datetime.now().year
        mock_now = datetime(current_year, 11, 8, 14, 0, 0)  # Saturday in November
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.098
    ...
    def test_winter_weekend_flat_rate_midnight(self, mock_datetime):
        """Test winter weekend flat rate at midnight."""
        # Saturday, November 8, 2025, 12:00 AM (winter, weekend, hour 0)
        current_year = datetime.now().year
        mock_now = datetime(current_year, 11, 8, 0, 0, 0)  # Saturday in November
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.098
    ...
    def test_summer_weekday_off_peak_morning(self, mock_datetime):
        """Test summer weekday off-peak morning rate."""
        # Monday, June 10, 2024, 6:00 AM (summer, weekday, hour 6)
        mock_now = datetime(2024, 6, 10, 6, 0, 0)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
    ...
    def test_summer_weekday_peak_morning(self, mock_datetime):
        """Test summer weekday peak morning rate (7-10)."""
        # Monday, June 10, 2024, 8:00 AM (summer, weekday, hour 8)
        mock_now = datetime(2024, 6, 10, 8, 0, 0)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
    ...
    def test_summer_weekday_peak_afternoon(self, mock_datetime):
        """Test summer weekday peak afternoon rate (11-16)."""
        # Monday, June 10, 2024, 1:00 PM (summer, weekday, hour 13)
        mock_now = datetime(2024, 6, 10, 13, 0, 0)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
    ...
    def test_summer_weekday_partial_peak_evening(self, mock_datetime):
        """Test summer weekday partial peak evening rate (17-18)."""
        # Monday, June 10, 2024, 6:00 PM (summer, weekday, hour 18)
        mock_now = datetime(2024, 6, 10, 18, 0, 0)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.122
    ...
    def test_summer_weekday_off_peak_night(self, mock_datetime):
        """Test summer weekday off-peak night rate."""
        # Monday, June 10, 2024, 11:00 PM (summer, weekday, hour 23)
        mock_now = datetime(2024, 6, 10, 23, 0, 0)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.076
    ...
    def test_summer_weekend_flat_rate(self, mock_datetime):
        """Test summer weekend has flat rate all day."""
        # Sunday, June 9, 2024, 2:00 PM (summer, weekend, hour 14)
        mock_now = datetime(2024, 6, 9, 14, 0, 0)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.076
    ...
    def test_summer_weekend_flat_rate_evening(self, mock_datetime):
        """Test summer weekend flat rate in evening."""
        # Sunday, June 9, 2024, 8:00 PM (summer, weekend, hour 20)
        mock_now = datetime(2024, 6, 9, 20, 0, 0)
        mock_datetime.now.return_value = mock_now
        mock_datetime.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        pricing = TimeOfUseElectricityPricing()
        assert pricing.get_current_price() == 0.076
    ...
    def test_transition_from_summer_to_winter(self, mock_datetime):
        """Test transition from summer to winter pricing."""
        ...
        # October 20 should be summer peak: 0.122 (weekday)
        assert oct_20_price == 0.122
        # November 20 should be winter weekday peak: 0.203 (weekday)
    ...
    def test_transition_from_winter_to_summer(self, mock_datetime):
        """Test transition from winter to summer pricing."""
